...
void WriteSetting(string settingComment, string settingName, string settingValue, FILE* fp)
{
	/*;Длина списка по умолчанию
      ListDefaultLength=9*/;
	fputc(COMMENT_SYMBOL, fp);
	fputs(settingComment.c_str(), fp);
	fputc('\n', fp);
	fputs(settingName.c_str(), fp);
	fputc(COMMENT_SEPARATOR, fp);
	fputs(settingValue.c_str(), fp);
	fputc('\n', fp);
}
